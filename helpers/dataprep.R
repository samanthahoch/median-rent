...
setwd("Z:\\Work\\Coop\\Fall_2019\\AdaptingWFExp\\data")
...
prep_rent_data <- function(file_name = "FY2018_50_County_rev.csv") {
  ...
  county_data <- st_read("county\\counties_westernSts_4326.shp")
  st_geometry(county_data) <- NULL
  
  # give rent data county ID which matches object ID in county
  rent_data <- left_join(rent_data, county_data[, c("CNTYID", "NAME", "Code")], by = c("name" = "NAME", "state" = "Code"))
  
  # make sure that all the rent data got a cntyid
  no_id <- nrow(rent_data[is.na(rent_data$CNTYID), ])
  if (no_id > 0) {
    stop(paste(no_id, "counties did not recieve an ID", paste(rent_data[is.na(rent_data$CNTYID), "name"], collapse = ", ")))
  } else {
...
prep_county_data <- function(file_name = "UNITEDSTATES_County_2018_ESRI_R2019.shp") {
  ...
  colnames(county_data)[colnames(county_data)=="OBJECTID"] <- "CNTYID"
  ...
  st_write(county_data, paste0("county\\counties_westernSts_4326.shp"))
